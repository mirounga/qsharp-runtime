...
if (($IsWindows) -or ((Test-Path Env:AGENT_OS) -and ($Env:AGENT_OS.StartsWith("Win"))))
{
    ...
    Write-Host 'cmake -DBUILD_SHARED_LIBS:BOOL="1" -DCMAKE_BUILD_TYPE=$Env:NATIVE_SIMULATOR_BUILD_CONFIGURATION ..'
    ...
    Write-Host 'cmake -DBUILD_SHARED_LIBS:BOOL="1" -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DCMAKE_BUILD_TYPE=$Env:NATIVE_SIMULATOR_BUILD_CONFIGURATION ..'
    cmake -DBUILD_SHARED_LIBS:BOOL="1" -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DCMAKE_BUILD_TYPE=$Env:NATIVE_SIMULATOR_BUILD_CONFIGURATION ..
}
elseif (($IsMacOS) -or ((Test-Path Env:AGENT_OS) -and ($Env:AGENT_OS.StartsWith("Darwin")))) {
    ...
    Write-Host 'cmake -DBUILD_SHARED_LIBS:BOOL="1" -DCMAKE_C_COMPILER=gcc-7 -DCMAKE_CXX_COMPILER=g++-7 -DCMAKE_BUILD_TYPE=$Env:NATIVE_SIMULATOR_BUILD_CONFIGURATION ..'
...
Write-Host 'cmake --build . --config $Env:NATIVE_SIMULATOR_BUILD_CONFIGURATION --target install'
