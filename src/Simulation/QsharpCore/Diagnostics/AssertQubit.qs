...
namespace Microsoft.Quantum.Diagnostics {
    ...
    operation AssertQubit (expected : Result, q : Qubit) : Unit is Adj {
    ...
    /// <xref:microsoft.quantum.diagnostics.assertqubitisinstatewithintolerance> allows for asserting
    /// arbitrary qubit states rather than only $Z$ eigenstates.
    operation AssertQubitWithinTolerance(expected : Result, q : Qubit, tolerance : Double) : Unit is Adj {
    ...
    operation AssertQubitIsInStateWithinTolerance(expected : (Complex, Complex), register : Qubit, tolerance : Double) : Unit is Adj {
        let (a, b) = expected;
        let (reA, imA) = a!;
        let (reB, imB) = b!;
